...
In this scenario any STAC metadata exists purely for discovery and cannot be used for filtering or subsetting (although that might be possible to support in the future by adding predicate push-down to [xpystac][1]).

### Straight to xarray

The best approach for accessing the data is to construct the lazily-loaded data cube in xarray and filter from there.
...
This is likely to be very fast if there is a consolidated metadata file OR the data is in Zarr-3 and the metadata fetch is highly parallelized ([read more][4]).
...
Once you have a list of assets that meet your needs you can use `xr.open_mfdataset` with the `zarr` or `stac` backend to construct the data-cube. 
